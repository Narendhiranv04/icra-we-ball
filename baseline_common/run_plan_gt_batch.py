"""Run planning-to-GT baseline trials over Kitchen or Living Room variants."""
...
def build_parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument("--environment", choices=("kitchen", "living_room"), required=True)
    parser.add_argument(
...
def main() -> None:
    ...
    allowed_methods = {"vlm_tamp", "owl_tamp"}
    if not set(arguments.methods) <= allowed_methods:
        parser.error("--methods supports only vlm_tamp,owl_tamp")
    ...
    if arguments.resume and summary_path.is_file():
        ...
        for row in previous.get("runs", ()):
            key = (
                str(row["method"]), str(row["environment"]), str(row["variant"]),
            ...
            for camera_count in camera_counts:
                for seed in seeds:
                    ...
                    if arguments.resume and result_path.is_file():
                        ...
                        key = (
                            method, arguments.environment, variant,
                            camera_count, seed,
                        )
                        rows_by_key[key] = {
                            "method": method,
