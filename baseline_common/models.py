"""Observation and action contracts shared by comparison baselines."""
...
class Observation:
    ...
    def as_semantic_neutral_prompt_dict(self) -> dict[str, Any]:
        """Textualize only shared observable identity and state.

        This representation omits annotation aliases. Use
        ``as_annotated_prompt_dict`` with alias-labelled RGB views.
        """
        allowed_facts = {"source_region", "location", "region_id"}
        return {
            "scene": self.scene,
            "revision": self.revision,
            "visible_objects": [
    ...
    def as_annotated_prompt_dict(self) -> dict[str, Any]:
        """Observable state plus the aliases printed on supplied RGB views."""
        result = self.as_semantic_neutral_prompt_dict()
        result["semantic_annotations"] = {
            "objects": [
                {"id": item.entity_id, "alias": item.label}
                for item in self.entities
            ],
            "regions": [
                {"id": item.region_id, "alias": item.label}
                for item in self.regions
            ],
        }
        return result
...
class ActionResult:
    ...
    @classmethod
    def succeeded(cls, *effects: str) -> "ActionResult":
        return cls(True, effects=tuple(effects))
...
def _parse_action(
    ...
    for name, reference_kind in expected.items():
        ...
        if reference_kind == "object":
            allowed = observation.object_ids
        elif reference_kind == "region":
            allowed = observation.region_ids
        elif reference_kind == "destination":
            allowed = observation.object_ids | observation.region_ids
        else:
            raise ValidationError(
                f"Action {skill} has unsupported reference kind {reference_kind!r}"
            )
