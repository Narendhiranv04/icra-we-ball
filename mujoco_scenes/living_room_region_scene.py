...
def _apply_movie_night_visuals(root: ET.Element, ablation2: bool) -> None:
    ...
    if ablation2:
        ...
        for name, position, mesh, material in (
            ...
            ET.SubElement(
                body, "geom",
                {
                    "name": f"{name}_collision", "type": "box",
                    "size": "0.065 0.065 0.025", "rgba": "0 0 0 0",
                    "mass": "0.22", "group": "3",
                },
            )
    else:
    ...
    for body_name, (mesh, material, position) in replacements.items():
        ...
        for geom in body.findall("geom"):
            ...
            geom.set("group", "3")
            geom.attrib.pop("material", None)
            if body_name in {"a2_drink_left", "a2_drink_right"}:
                geom.set("contype", "0")
                geom.set("conaffinity", "0")
                geom.set("mass", "0")
        if body_name in {"a2_drink_left", "a2_drink_right"}:
            # The original cylinder-plus-handle proxy rocks indefinitely on
            # MuJoCo's flat contact manifold. A hidden square-footprint proxy
            # keeps the free payload stable without changing rendered RGB-D
            # geometry or any ablation measurement.
            ET.SubElement(
                body,
                "geom",
                {
                    "name": f"{body_name}_stable_collision",
                    "type": "box",
                    "size": "0.055 0.055 0.070",
                    "rgba": "0 0 0 0",
                    "mass": "0.38",
                    "group": "3",
                },
