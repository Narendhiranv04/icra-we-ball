...
class KitchenPhaseCExecutionDispatcher:
    ...
    def __init__(
        ...
        if not isinstance(frozen_registry, dict) or "objects" not in frozen_registry:
            raise ValueError("Frozen registry must contain objects")
        objects = frozen_registry["objects"]
        self.registry_by_id = (
            objects if isinstance(objects, dict)
            else {
                row["generic_object_id"]: row
                for row in objects
                if isinstance(row, dict) and "generic_object_id" in row
            }
        )
        if not isinstance(objects, (dict, list)) or len(self.registry_by_id) != len(objects):
            raise ValueError("Frozen registry contains malformed or duplicate objects")
        self.post_pick_carry_arm_by_id: dict[str, np.ndarray] = {}
        self.authorize_plan(frozen_plan)

    def authorize_plan(self, plan: list[dict[str, Any]]) -> None:
        """Freeze a newly refined symbolic plan before physical execution."""
        ...
        for index, row in enumerate(plan, start=1):
            ...
            raw_step = row.get("step", index)
            if isinstance(raw_step, bool):
                raise ValueError("Frozen plan step must be an integer")
            try:
                step = int(raw_step)
            except (TypeError, ValueError) as error:
                raise ValueError("Frozen plan step must be an integer") from error
            if step in seen_steps:
                raise ValueError(f"Duplicate frozen plan step: {step}")
        ...
        self.authoritative_frozen_plan = normalized
        self.frozen_plan = phase_c_execution_plan(
            normalized, {"objects": self.registry_by_id}
        )
        self.ledger = PhaseCExecutionLedger(self.frozen_plan)
        self.expected_pairs: dict[str, dict[tuple[str, str], int]] = {
            "POUR": {}, "STIR": {}
        }
        for row in self.frozen_plan:
            operator = row["action"]
            if operator not in self.expected_pairs:
                continue
            pair = tuple(row["arguments"][:2])
            if len(pair) != 2:
                raise ValueError(f"{operator} requires a source/tool and target")
            if pair in self.expected_pairs[operator]:
                raise ValueError(f"Duplicate frozen {operator} pair: {pair}")
            self.expected_pairs[operator][pair] = row["step"]
    ...
    def execute_phase2_action(self, action: dict[str, Any]) -> dict[str, Any]:
        ...
        if not isinstance(raw_arguments, list) or any(
            ...
            return {
                "request": {"action": operator, "arguments": raw_arguments},
                "success": False,
                "status": "MALFORMED_ACTION",
                "symbolic_effects_applied": False,
            }
        ...
        expected_arities = {
            ...
            "STIR": {2},
            "OPEN": {1},
            "PLACE_SERVING_UTENSIL": {2},
            "SERVE_COFFEE": {1},
            "SERVE_SOUP": {1},
        }
        if operator in expected_arities and len(arguments) not in expected_arities[operator]:
            return {
