...
def associate_segmented_centroid(
    label: str,
    centroid_world_m: np.ndarray,
    *,
    track_centroids: dict[str, np.ndarray],
    track_labels: dict[str, str],
    track_observations: dict[str, int],
    used_track_ids: set[str],
    maximum_distance_m: float,
) -> str:
    """Associate a frame-local learned mask using label and RGB-D position."""
    centroid = np.asarray(centroid_world_m, dtype=np.float64)
    candidates = []
    for track_id, previous in track_centroids.items():
        if track_id in used_track_ids or track_labels[track_id] != label:
            continue
        distance = float(np.linalg.norm(previous - centroid))
        if distance <= maximum_distance_m:
            candidates.append((distance, track_id))
    if candidates:
        _distance, track_id = min(candidates)
        count = track_observations[track_id]
        track_centroids[track_id] = (
            track_centroids[track_id] * count + centroid
        ) / (count + 1)
        track_observations[track_id] = count + 1
    else:
        track_id = f"object_{len(track_centroids) + 1:04d}"
        track_centroids[track_id] = centroid
