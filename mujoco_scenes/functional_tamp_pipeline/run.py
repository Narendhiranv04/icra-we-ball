...
def _write_run_manifest(state: _RunState) -> None:
    ...
    manifest = {
        ...
        "semantic_vlm_requests": 1 if (state.mode == "vlm" and state.spec_acquisition == "live_provider") else 0,
        "vlm_request_count": 1 if (state.mode == "vlm" and state.spec_acquisition == "live_provider") else 0,
        "astar_invocations": 1 if (state.candidate_plan or state.candidate_search_statistics) else 0,
