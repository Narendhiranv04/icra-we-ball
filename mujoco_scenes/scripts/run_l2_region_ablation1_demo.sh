...
run_id="${1:-l2_living_room_region_ablation1_demo}"
...
echo "Running actual L2 RGB-D + YOLO-World region benchmark: $run_id"
docker run --rm \
    --user "$(id -u):$(id -g)" \
    -e HOME=/tmp \
    -e MUJOCO_GL=egl \
...
echo "Generating matrix, graphs, stage views, GIF, MP4, HTML, and README"
docker run --rm \
    --user "$(id -u):$(id -g)" \
    -e HOME=/tmp \
    --entrypoint python \
    ...
    "$image" \
    -m mujoco_scenes.generate_region_ablation_report \
    "/runs/$run_id" \
...
docker run --rm \
    ...
    "$image" -c \
    "import json,sys; v=json.load(open('/runs/$run_id/region_ablation_validation.json')); r=json.load(open('/reports/$run_id/report_data.json')); sys.exit(0 if v['passed'] and r['animation']['mp4'] else 2)"
