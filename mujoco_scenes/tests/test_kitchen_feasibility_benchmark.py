...
def _assignment_graph(coffee_edges):
    ...
    soup_tools = ["soup_tool_1", "soup_tool_2"]
...
def test_coffee_one_tool_cover_is_preferred_over_multi_tool_alternatives():
    ...
    assert group["complete_target_coverage_exists"]
...
def test_coffee_two_tool_collective_cover_is_feasible():
    result = _evaluate({
        "a": {COFFEE[0]}, "b": {COFFEE[1]},
    })
    group = _coffee_group(result)
    assert result["status"] == "COMPLETE"
    assert group["minimum_distinct_tool_count"] == 2
...
def test_oracle_curated_variants_and_stage_labels_match_intent():
    ...
    assert results["F0_ALL_VISIBLE"]["oracle_coffee_minimum_unique_tools"] == 1
    assert results["F0_ALL_VISIBLE"]["oracle_earliest_feasible_stage"] == "INITIAL"
    assert results["F1_HIDDEN_COFFEE_VESSEL"]["oracle_earliest_feasible_stage"] == "C2"
    assert results["F2_HIDDEN_SOUP_BOWL"]["oracle_earliest_feasible_stage"] == "B1"
    assert results["F4_TOOLS_IN_DRAWERS"]["oracle_earliest_feasible_stage"] == "D2"
    assert results["F5_FULL_DISTRIBUTED_SEARCH"]["oracle_earliest_feasible_stage"] == "C1"
    assert results["I3_MISSING_SOUP_UTENSIL"]["oracle_soup_matching_size"] == 1
...
def test_oracle_geometry_is_extracted_from_instantiated_model():
    ...
    shallow = extract_exact_object_geometry(
        scene, "ab3_narrow_deep_cup", "ab3_narrow_deep_cup", geometry_config=config
    )
    deep = extract_exact_object_geometry(
        scene, "ab3_medium_deep_mug", "ab3_medium_deep_mug", geometry_config=config
    )
    assert shallow.opening_width_m > 0
    assert deep.cavity_depth_m > 0
...
def test_assignment_driven_roles_omit_misleading_fixed_count():
    ...
    assert binding["selected_distinct_physical_objects"] == 2
...
def test_suite_contains_only_position_and_absence_variants():
    benchmark = load_feasibility_benchmark_config()
    assert len(benchmark["variants"]) == 12
    assert not any(name.startswith("P") for name in benchmark["variants"])
    assert not any("DECOY" in name or "TRAP" in name for name in benchmark["variants"])
