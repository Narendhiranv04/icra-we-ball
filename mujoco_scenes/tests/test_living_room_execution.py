...
def test_variant_registry_completeness():
    """Verify all 13 integrated variants are present and categorized."""
    assert len(EXPECTED_VARIANTS) == 13
    ...
    assert "F0_BASE" in feasible
    assert "F6_DECOY_SURPLUS" in feasible
    assert "I0_PERSONAL_SEMANTIC_DEFICIT" in infeasible
    assert "I5_CROSS_FUNCTION_CONFLICT" in infeasible


def test_variant_name_normalization():
    """Test variant name normalization."""
    assert normalize_variant_name("f0_base") == "F0_BASE"
    assert normalize_variant_name("F0") == "F0_BASE"
    assert normalize_variant_name("f3") == "F3_GLOBAL_MATCHING_REQUIRED"
    assert normalize_variant_name("I0_PERSONAL_SEMANTIC_DEFICIT") == "I0_PERSONAL_SEMANTIC_DEFICIT"
...
def test_feasible_variants_entity_resolution(phase1_root: Path, phase2_root: Path):
    """Verify entity resolution maps 6 objects and regions for all feasible variants."""
    ...
    for variant in feasible_variants:
        ...
        assert len(placements["placements"]) == 6
...
def test_provenance_mismatch_detection(phase1_root: Path, phase2_root: Path):
    ...
    p1_dir = phase1_root / "F0_BASE"
    p2_dir = phase2_root / "F1_LAYOUT_SWAPPED"
    with tempfile.TemporaryDirectory() as tmp_dir:
...
def test_5camera_recorder_and_manifest():
    ...
    scene = L2LivingRoomRegionScene("L2_integrated_living_room_region_function_F0_BASE", "google")
