...
LIVING_ROOM_CAMERAS = (
    ...
    "tv_camera",
    "table_camera",
    "inspection_oblique_camera",
    "wrist_camera",
    "head_camera_rgb",
)
LIVING_ROOM_POINT_CLOUD_CAMERAS = (
    "overhead_camera",
    "room_corner_camera",
    "tv_camera",
    "table_camera",
...
class LivingRoomObservationState:
    """Controller state exposed to the generic observed-state pipeline."""
...
class LivingRoomScene:
    ...
    point_cloud_cameras = LIVING_ROOM_POINT_CLOUD_CAMERAS
    ...
    def get_visible_object_instances(self) -> list[tuple[str, str]]:
        """Return visible rigid props for generic instance-mask association."""
        return [(object_name, object_name) for object_name in LIVING_ROOM_OBJECTS]

    def get_instance_source_region(self, instance_name: str) -> str | None:
        """Expose only the region assigned after an object is visibly present."""
        return "room" if instance_name in LIVING_ROOM_OBJECTS else None

    def inspection_source_region(self, region_id: str) -> str | None:
        """Select source gating appropriate to this room inspection stage."""
        return "room" if region_id == "INITIAL" else region_id

    def get_region_observation_states(self) -> dict[str, dict]:
        """Return drawer state without claiming any hidden drawer contents."""
        return {
            region_id: {
                "region_id": region_id,
    ...
    def open_container(self, region_id: str, steps: int = 1000) -> list[str]:
        """Directly open a room drawer for deterministic virtual inspection."""
    ...
    def close_container(self, region_id: str, steps: int = 1000) -> None:
        """Directly close a room drawer and settle the rigid scene."""
        actuator_id = self._drawer_actuator_id(region_id)
    ...
    def set_all_containers_open_snapshot(self) -> None:
        """Open both drawers while preserving observed-state bookkeeping."""
        for region_id in LIVING_ROOM_REGIONS:
    ...
    def print_scene_summary(self) -> None:
        ...
        print(f"Goal:  {self.goal}")
        print("-" * 60)
        print(f"Visible objects:     {sorted(LIVING_ROOM_OBJECTS)}")
